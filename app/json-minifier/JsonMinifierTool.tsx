...
import { minifyJSON } from "@/lib/tools/developer";
...
export default function JsonMinifierTool() {
  ...
  const handleProcess = () => {
    if (!input.trim()) {
      setError("LÃ¼tfen sÄ±kÄ±ÅŸtÄ±rmak iÃ§in bir JSON verisi girin.");
      setOutput("");
      return;
    }
    ...
    try {
      ...
      setError("GeÃ§ersiz JSON formatÄ±. LÃ¼tfen kontrol edip tekrar deneyin.");
      setOutput("");
    }
  };
      ...
      {output && (
        <div className="space-y-1">
          <label className="text-sm font-medium text-gray-700 dark:text-gray-300">
            SÄ±kÄ±ÅŸtÄ±rÄ±lmÄ±ÅŸ JSON
          </label>
          <div className="relative">
            <textarea
              value={output}
              readOnly
              className="w-full rounded-lg border border-gray-300 bg-gray-50 p-4 pr-28 font-mono text-sm text-gray-900 focus:outline-none dark:border-gray-700 dark:bg-gray-800/60 dark:text-gray-100"
              rows={6}
            />
            <button
              type="button"
